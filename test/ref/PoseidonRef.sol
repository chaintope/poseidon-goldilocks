...
/// @title Poseidon-Goldilocks reference (solc) implementation — TEST ORACLE ONLY.
///
/// This is the original solc PGStage1 + PGStage2 libraries. Production ships the hand-written Yul
/// stages (yul/Stage*.hex) wired into src/PoseidonGoldilocks.sol; this solc port is kept ONLY in the
/// test tree as the differential oracle for the Yul stages: YulStage1.t.sol / YulStage2.t.sol fuzz the
/// Yul output against PGStage1.run / PGStage2.run over random valid inputs (256+ runs). It is never
/// deployed in production. See reference/poseidon_reference.py for the independent Python source of truth.
///
/// The 30 rounds are split so each library stays < EIP-170 24KB:
